          ...
          <table className="min-w-full divide-y divide-white/20">
            <thead>
              <tr>
                ...
                {showAll && <th className="card-header px-5 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Requester</th>}
              ...
              {tickets.length === 0 ? (
                ...
                tickets.map((t) => (
                  <tr key={t.id} className="table-row-glass transition-colors">
                    ...
                    {showAll && <td className="px-5 py-4 text-sm text-slate-600">{t.requester?.email ?? "—"}</td>}
